"""
This file is largely borrowed from skywork-o1-prm-inference (https://github.com/SkyworkAI/skywork-o1-prm-inference)
"""
...
def prepare_input(problem, response, tokenizer, step_tag, step_tag_id):
    prompt_ids = tokenizer.encode(tokenizer.bos_token + problem + "\n")
    response_ids = []
    steps = []
    reward_flags = [0] * len(prompt_ids)
    for idx, step in enumerate(response.split(step_tag)):
        if step != "":
            step_ids = tokenizer.encode(step)
        else:
            step_ids = []
        step_ids += [step_tag_id]
        step = step + step_tag
        flag = [0] * len(step_ids)
        flag[-1] = 1
...
def derive_step_rewards(rewards, reward_flags):
    ...
    for i in range(batch_size):
        ...
        step_rewards = rewards[i][rewards_indices]
