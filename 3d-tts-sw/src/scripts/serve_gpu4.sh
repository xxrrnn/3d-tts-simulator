...
do
    ...
    command="VLLM_WORKER_MULTIPROC_METHOD=spawn CUDA_VISIBLE_DEVICES=${GPU_LIST[$i-$NUM_RM_WORKER+2]},${GPU_LIST[$i-$NUM_RM_WORKER+3]} ${PYTHON_EXECUTABLE} -m reason.llm_service.workers.vllm_worker --max_model_length ${max_model_length} --num-gpus ${tensor_parallel_size} --gpu_memory_utilization ${gpu_memory_utilization} --enable_chunked_prefill --swap_space 16 --model-path $POLICY_MODEL_PATH --controller-address http://$HOST_ADDR:$CONTROLLER_PORT --host $HOST_ADDR --port $WORKER_PORT --worker-address http://$HOST_ADDR:$WORKER_PORT"
    ...
    tmux send-keys "$command" Enter
    echo "Policy worker $i started on GPU ${GPU_LIST[$i-$NUM_RM_WORKER+2]},${GPU_LIST[$i-$NUM_RM_WORKER+3]} with port $WORKER_PORT, model: $POLICY_MODEL_PATH"
done
