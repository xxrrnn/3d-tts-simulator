...
# 使用 tmux 内 conda activate（见下方 CONDA_ENV_NAME）
...
do
    ...
    tmux send-keys "$command" Enter
    echo "Policy worker $i started on GPU ${GPU_LIST[$i-$NUM_RM_WORKER+1]},${GPU_LIST[$i-$NUM_RM_WORKER+2]} with port $WORKER_PORT, model: $POLICY_MODEL_PATH"
done
