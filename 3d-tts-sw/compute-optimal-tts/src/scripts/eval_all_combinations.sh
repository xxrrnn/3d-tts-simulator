...
start_services() {
    ...
    # 后台启动服务 (使用4GPU配置)
    bash scripts/serve_gpu1.sh "$policy_path" "$reward_path" "$HOST_ADDR" "$CONTROLLER_PORT" "$WORKER_BASE_PORT" &
    SERVICE_PID=$!
...
cleanup_output_dir() {
    ...
    local output_dir="${OUTPUT_BASE_DIR}/${task}_beam_search/${policy_model}/${reward_model}/256_${branch_width}_1"

    if [ -d "$output_dir" ]; then
        log_message "删除不完整输出目录: $output_dir"
        rm -rf "$output_dir"
    fi
}
    ...
    while [ $attempt -le $max_retries ]; do
        ...
        if [ $exit_code -eq 0 ]; then
            ...
            if [ $exit_code -eq 124 ] || [ $exit_code -eq 137 ]; then
                log_message "检测到评估卡死/超时: ${task}, Branch=${branch_width} - 退出码: ${exit_code}"
            else
                log_message "评估失败: ${task}, Branch=${branch_width} - 退出码: ${exit_code}"
            fi

            # 失败或卡死时：停止服务 -> 删除不完整输出 -> 重启服务
            stop_services
            cleanup_output_dir "$policy_path" "$reward_path" "$task" "$branch_width"
