...
class TreeSearchSolutionOutput(SolutionOutput):
    ...
    token_prob_history: List[List[float]]
...
class MathEvaluator:
    ...
    def evaluate_problem(self, problem_inst: Dict[str, str], solver_fn: Callable) -> List[str]:
        ...
        token_prob_history = solution.token_prob_history
        ...
        result, output = self.analyze_output(
            problem_inst, solution.solutions, reward_history, token_history, prob_history, model_history, token_prob_history
        )
        ...
        for i, o in enumerate(output):
            ...
            if isinstance(solution, TreeSearchSolutionOutput):
                ...
                ### beam search start
                # 添加详细beam search日志信息
                if solution.detailed_beam_search_logs and i < len(solution.detailed_beam_search_logs):
                    if solution.detailed_beam_search_logs[i] is not None:
                        o["detailed_beam_search_log"] = solution.detailed_beam_search_logs[i]
                if solution.final_path_infos and i < len(solution.final_path_infos):
                    if solution.final_path_infos[i] is not None:
                        o["final_path_info"] = solution.final_path_infos[i]
    ...
    def analyze_output(
        self, problem_inst: Dict[str, str], gen_answers: List[str], reward_history, token_history, prob_history,
        model_history=None, token_prob_history=None
    ):
        ...
        output_list = [
            ...
            for i, (txt, v, extracted_answer, reward, token, prob, model, token_prob) in
            enumerate(
                zip(
                    gen_answers, value_list, extracted_answers, reward_history, token_history, prob_history, model_history,
                    token_prob_history if token_prob_history is not None else [[] for _ in gen_answers]
                )
            )
        ]
        res = {
