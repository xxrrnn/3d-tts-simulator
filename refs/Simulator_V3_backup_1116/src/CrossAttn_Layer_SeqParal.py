...
#无预测，无稀疏
class CrossAttn_Layer_SeqParal:
    def __init__(self,
        stage_id:int,
    ...
    def init_matrices(self):
        ...
        self.generated_chunk_q_per_core = myMatrix(
                data_width    = 8,
                matrix_height = self.N1//4,
                matrix_width  = self.head_dim,
            )
        self.generated_k_per_core = myMatrix(
                data_width    = 8,
                matrix_height = self.N2,
                matrix_width  = self.head_dim,
            )
        self.generated_v_per_core = myMatrix(
                data_width    = 8,
                matrix_height = self.N2,
                matrix_width  = self.head_dim,
            )
...
#####################################################################################
    ...
    def q_gen_per_core(self):
        ...
        load_in1token_from_dram = self.input1_matrix_per_core.total_KB()
        store_gened_part_q_to_dram = self.generated_part_q_per_core.total_KB()

        return matmul_workload, load_in1token_from_dram, load_weight_q_from_dram, store_gened_part_q_to_dram
    ...
    def k_gen_per_core(self):
        ...
        load_in2token_from_dram = self.input2_matrix_per_core.total_KB()
        store_gened_part_k_to_dram = self.generated_part_k_per_core.total_KB()

        return matmul_workload, load_in2token_from_dram, load_weight_k_from_dram, store_gened_part_k_to_dram
    ...
    def v_gen_per_core(self):
        ...
        return matmul_workload, load_in2token_from_dram, load_weight_v_from_dram, store_gened_part_v_to_dram
    
    def q_all2all_transfer(self):
        logger.info("start q_all2all_transfer")
        return (3/4) * self.generated_chunk_q_per_core.total_KB()
    
    def k_allgather_transfer(self):
        logger.info("start k_allgather_transfer")
        return (3) * self.generated_part_k_per_core.total_KB()

    def v_allgather_transfer(self):
        logger.info("start v_allgather_transfer")
        return (3) * self.generated_part_v_per_core.total_KB()
    ...
    def chunk_s_gen_per_core(self):
        ...
        matmul_workload = MatMul_Workload(
            ...
            matrix_A = self.generated_k_per_core,
        ...
        load_gened_k_from_dram = self.generated_k_per_core.total_KB()
        # transfer_gened_k_from_noc = 3*self.generated_k_per_core.total_KB() #for ring attention
        store_chunk_s_to_dram = self.generated_chunk_s_per_core.total_KB()

        return matmul_workload, load_gened_chunk_q_from_dram, load_gened_k_from_dram, store_chunk_s_to_dram
    ...
    def sv_gen_per_core(self):
        ...
        matmul_workload = MatMul_Workload(
            ...
            matrix_B = self.generated_v_per_core,
        )
        assert matmul_workload.matrix_C == self.generated_chunk_sv_per_core, "sv生成的矩阵C的维度必须等于generated_chunk_sv_per_core"
        
        load_gened_chunk_s_from_dram = self.generated_chunk_s_per_core.total_KB()
        load_gened_v_from_dram = self.generated_v_per_core.total_KB()
        ...
        return matmul_workload, load_gened_chunk_s_from_dram, load_gened_v_from_dram, store_chunk_sv_to_dram
