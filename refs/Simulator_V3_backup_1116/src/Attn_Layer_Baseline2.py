"""
这个Baseline是不用3D堆叠DRAM，而采用HBM
也就是2D的加速器

注意，此时不需要core2core的通信以同步数据
只需要从共享的DRAM中读取数据即可



准确地说，是allreduce = reducescatter+allgather中，第二步allgather不再需要
"""
import logging
...
class Attn_Layer_Baseline2:
    ...
    def init_matrices(self):
        ...
        self.cached_part_k_per_core = myMatrix(
                data_width    = 8,
                matrix_height = self.M,
                matrix_width  = self.head_dim//4,
            )

        self.cached_part_v_per_core = myMatrix(
                data_width    = 8,
                matrix_height = self.M,
                matrix_width  = self.head_dim//4,
            )

        self.concat_part_k_per_core = myMatrix(
                data_width    = 8,
                matrix_height = self.N + self.M,
                matrix_width  = self.head_dim//4,
            )

        self.concat_part_v_per_core = myMatrix(
                data_width    = 8,
                matrix_height = self.N + self.M,
                matrix_width  = self.head_dim//4,
            )
...
#####################################################################################
    ...
    def psum_s_gen_per_core(self):
        ...
        matmul_workload = MatMul_Workload(
            ...
            matrix_A = self.concat_part_k_per_core,
        ...
        load_concat_part_k_from_dram = self.concat_part_k_per_core.total_KB()
        ...
        return matmul_workload, load_gened_part_q_from_dram, load_concat_part_k_from_dram, store_psum_s_to_dram
    ...
    def sv_gen_per_core(self):
        ...
        matmul_workload = MatMul_Workload(
            ...
            matrix_B = self.concat_part_v_per_core,
        )
        ...
        load_concat_part_v_from_dram = self.concat_part_v_per_core.total_KB()
        store_part_sv_to_dram = self.generated_part_sv_per_core.total_KB()

        return matmul_workload, load_s_from_dram, load_concat_part_v_from_dram, store_part_sv_to_dram
