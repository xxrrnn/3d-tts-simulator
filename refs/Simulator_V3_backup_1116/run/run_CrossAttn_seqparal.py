...
def run_CrossAttn_seqparal(
    my_attn_layer: CrossAttn_Layer_SeqParal,
    ...
    运行CrossAttn序列并行模型
    """
    q_gen_matmul, load_in1token_from_dram_for_q_gen, load_weight_q_from_dram, store_gened_part_q_to_dram  = my_attn_layer.q_gen_per_core()
    k_gen_matmul, load_in2token_from_dram_for_k_gen, load_weight_k_from_dram, store_gened_part_k_to_dram  = my_attn_layer.k_gen_per_core()
    v_gen_matmul, load_in2token_from_dram_for_v_gen, load_weight_v_from_dram, store_gened_part_v_to_dram  = my_attn_layer.v_gen_per_core()

    q_all2all_noc_per_core = my_attn_layer.q_all2all_transfer()
    k_allgather_noc_per_core = my_attn_layer.k_allgather_transfer()
    v_allgather_noc_per_core = my_attn_layer.v_allgather_transfer()

    s_gen_matmul, load_gened_chunk_q_from_dram, load_gened_k_from_dram , store_chunk_s_to_dram = my_attn_layer.chunk_s_gen_per_core()
    
    sv_gen_matmul, load_chunk_s_from_dram, load_gened_v_from_dram,  store_chunk_sv_to_dram  = my_attn_layer.sv_gen_per_core()
    ...
    q_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_in1token_from_dram_for_q_gen + load_weight_q_from_dram + store_gened_part_q_to_dram,
        dram_config=my_dram_config,
    )
    k_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_in2token_from_dram_for_k_gen + load_weight_k_from_dram + store_gened_part_k_to_dram,
        dram_config=my_dram_config,
    )
    v_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_in2token_from_dram_for_v_gen + load_weight_v_from_dram + store_gened_part_v_to_dram,
        dram_config=my_dram_config,
    )

    s_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_gened_chunk_q_from_dram + load_gened_k_from_dram + store_chunk_s_to_dram,
        dram_config=my_dram_config,
    )


    sv_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_chunk_s_from_dram + load_gened_v_from_dram + store_chunk_sv_to_dram,
        dram_config=my_dram_config,
    )
    ...
    k_allgather_noc_latency_computer = NoC_Latency_Computer(
        transfer_workload=k_allgather_noc_per_core,
        noc_config=my_noc_config,
    )

    v_allgather_noc_latency_computer = NoC_Latency_Computer(
        transfer_workload=v_allgather_noc_per_core,
        noc_config=my_noc_config,
    )
    ...
    noc_latency = q_all2all_noc_latency_computer.transfer_latency() + k_allgather_noc_latency_computer.transfer_latency() + v_allgather_noc_latency_computer.transfer_latency() +  sv_all2all_noc_latency_computer.transfer_latency() + o_allreduce_noc_latency_computer.transfer_latency()

    total_latency = computing_latency + dram_latency + noc_latency + non_linear_latency

    logger.info(f"CrossAttn序列并行模型非线性延迟: {non_linear_latency}")

    logger.info(f"CrossAttn序列并行模型计算延迟: {computing_latency}")
    logger.info(f"CrossAttn序列并行模型DRAM延迟: {dram_latency}")
    logger.info(f"CrossAttn序列并行模型NoC延迟: {noc_latency}")
    
    logger.info(f"CrossAttn序列并行模型总延迟: {total_latency}")
    ...
    noc_energy_consumption = 64*(q_all2all_noc_latency_computer.transfer_energy_consumption() + k_allgather_noc_latency_computer.transfer_energy_consumption() + v_allgather_noc_latency_computer.transfer_energy_consumption() + sv_all2all_noc_latency_computer.transfer_energy_consumption() + o_allreduce_noc_latency_computer.transfer_energy_consumption())
    sram_energy_consumption = 64*my_sram_buffer_config.average_power * total_latency
    ...
    logger.info(f"CrossAttn序列并行模型非线性能耗: {non_linear_energy_consumption}")
    logger.info(f"CrossAttn序列并行模型计算能耗: {computing_energy_consumption}")
    logger.info(f"CrossAttn序列并行模型DRAM能耗: {dram_energy_consumption}")
    logger.info(f"CrossAttn序列并行模型NoC能耗: {noc_energy_consumption}")
    logger.info(f"CrossAttn序列并行模型SRAM能耗: {sram_energy_consumption}")
    logger.info(f"CrossAttn序列并行模型总能耗: {total_energy_consumption}")
