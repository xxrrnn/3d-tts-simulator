...
setup_logger(root_dir, "run_Infinity2B_baseline")
...
def main_for_Infinity2B_baseline():
    logger.info("Infinity2B模型-基线模型运行开始")
    ...
    result_file_dir = os.path.join(result_dir, "infinity2B_baseline_1115.txt")
    
    with open(result_file_dir, "w") as f:   
        f.write(f"开始Infinity2B模型-基线模型运行\n")
    ...
    for i in range(Infinity2B_Model_config["layer_num"]):
        ...
        with open(result_file_dir, "a") as f:
            f.write(f"第{i}层延迟: {stage_latency[-1]}ms;")
            f.write(f"能耗: {stage_energy_consumption[-1]}mJ\n")
        
    logger.info(f"Infinity2B模型-基线模型总延迟: {sum(stage_latency)}ms")
    logger.info(f"Infinity2B模型-基线模型总能耗: {sum(stage_energy_consumption)}mJ\n")
