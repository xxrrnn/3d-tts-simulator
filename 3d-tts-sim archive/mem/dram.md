...
当前默认参数对齐 `refs/DRAMsim3/configs/LPDDR4_8Gb_x16_2400.ini`：
...
如果你的硬件是“4 个 die 锁步，同时访问 4 个 die 的相同 bank/row/column”，
则更适合把它看成一个“更宽的逻辑 bank”，而不是 4 个独立 bank。
这时建议直接使用：
...
而 `tRCD / tRP / tCL / tCWL / burst_cycles` 保持不变。
这对应“延迟基本不变，但有效带宽变为 4 倍”。

如果你想按上一张 LPDDR5 x16 BG mode 表来生成时序，也可以单独调用：

```python
from mem import DramTiming

timing = DramTiming.lpddr5_x16_bg_from_ck(
    800,
    t_cl=17,
    t_cwl=14,
)
...
它不是 JEDEC 参数，而是为了让这个 Python 模型在当前 LPDDR 配置下，与 `DRAMsim3` 的读请求回调周期对齐。
...
这里的“碎片化”更准确地说，是“按块 gather 访问”。
它不一定是很多小字节碎片，也可以是很多固定大小的数据块。

你的典型场景可以写成：

- 每个逻辑块大小固定为 `128B`
- 地址排列可能像 `ABCDABCDABCDABCBCB`
- 你只想取出所有 `B`
- 每个 `B` 的起始地址由你提供
- 这些地址是递增的
...
```python
report = dram.read_addresses(b_addresses, access_size=128)
```

其中 `b_addresses` 就是所有 `B` 的起始地址列表。

对于这类输入，模型的处理规则是：

- 如果两个被选中的块在地址上正好相邻，模块会自动把它们合并成一个更长的连续片段
- 因此“步长为 0”的情况不会被误算成碎片化
- 只有当两个被选中的块之间真的存在空洞时，才会形成新的 fragment

如果访问不是一个连续区间，而是很多分散的块，那么每个 fragment 都会单独发起事务和列访问。

碎片化的额外损失主要来自：

1. 重复事务开销

- 每个额外事务都会引入一次 `transaction_overhead`
- 这部分记录在 `transaction_overhead_cycles`

2. 重复列命令开销

- 读用 `tCL`
- 写用 `tCWL`
- 这部分记录在 `fragmentation_cycles`

3. 非 burst 对齐损失

- 如果一个碎片大小不是 `burst_bytes` 的整数倍，会有未用满 burst 的浪费
- 这部分记录在 `partial_burst_cycles`
- 如果你用的是默认 LPDDR 配置，并且每个块就是 `128B`，通常这一项为 `0`
- 如果你用的是 4-die 锁步配置，则块大小需要达到 `512B` 才会让这一项变成 `0`

4. 如果碎片落在不同 row

- 还会继续叠加 `tRP + tRCD`
- 这部分记录在 `cross_row_cycles`

这些额外量分别记录在：

- `transaction_overhead_cycles`
...
- 在当前 LPDDR 默认配置下，读请求能与 `DRAMsim3` 的回调周期对齐
...
dram = SimpleDramLatencyModel.lpddr4_x16_defaults()
...
```python
report = dram.read(
    addresses=[0x1000, 0x1100, 0x2000, 0x2100],
    access_size=128,
)
```
...
- 如果不传 `access_size`，默认按一个 burst，即 `128B`
- 如果相邻两个地址正好首尾相接，模块会自动把它们并成一个更长的连续 fragment
- 因此它很适合你这种“给定递增地址列表，按固定块大小抽取 B”的模式
...
### 8.2 `reset()` 的功能、原理和使用方法

`reset()` 的作用不是“只关闭当前 row”，而是把整个简化 DRAM 模型恢复到初始状态。

它在代码里的实际行为是：

- 清空当前 bank 状态
- 清空已经打开的 `open_row`
- 清空 bank 的 `ready_cycle`
- 把全局时间 `self._cycle` 直接置回 `0`

也就是说，`reset()` 之后模型等价于：

- 之前没有发生过任何读写
- 没有 row 处于打开状态
- 没有 write recovery 等待
- 下一次访问会从一个全新的、closed-row 的 bank 开始计算

因此 `reset()` 会同时影响两类信息：

1. row 命中状态

- reset 之后，之前的 row locality 全部丢失
- 下一次访问不会再继承上一次访问留下的 row hit

2. 时间连续性

- reset 之后，累计时间归零
- `report.start_cycle` 会重新从 `0` 开始
- 之前访问带来的等待时间不会再传到后续请求

`close_open_row()` 和 `reset()` 的区别是：

- `close_open_row()` 只清掉当前打开的 row
- 它不会清掉已经累计的时间 `self._cycle`
- 它更像“保留时间线，但强制下一次访问从 closed row 开始”
- `reset()` 则是“时间线和 bank 状态一起清空”

推荐这样使用 `reset()`：

- 当你要做两组独立实验对比时使用
- 当你想分别评估两种映射/存储策略，而不希望前一组结果污染后一组时使用
- 当你把每个阶段都视为独立 latency estimate，而不是同一条连续时间线时使用

不建议这样使用 `reset()`：

- 在同一个真实连续执行流中，每处理一小段请求就 reset
- 在你希望保留 row hit / row conflict 历史的情况下 reset
- 在你希望写后的 `write_recovery` 继续影响后继请求时 reset

结合你当前 `dram_test.py` 的语义：

- 在 `naive save` 和 `chunk save` 两段之间调用 `dram.reset()` 是合理的
- 因为这表示你想把两种保存策略当成两组独立实验来比较
- 但如果你想模拟“同一个 decode 过程中连续发生的真实访存”，中间就不应该 reset
...
```python
dram = SimpleDramLatencyModel.lpddr4_x16_lockstep_defaults(4)
r0 = dram.read_addresses(addrs_a, access_size=128)
r1 = dram.read_addresses(addrs_b, access_size=128)
```

这时 `r1` 会继承 `r0` 留下的 open row 和时间线。

2. 只切断 row 命中，不切断时间线

```python
dram.close_open_row()
r2 = dram.read_addresses(addrs_c, access_size=128)
```

这时 `r2` 会从 closed row 开始，但时间仍接在前面。

3. 完全重新开始

```python
dram.reset()
r3 = dram.read_addresses(addrs_d, access_size=128)
```

这时 `r3` 会从 cycle 0、closed row、空 bank 状态重新计算。


### 8.3 当前模型是单 bank 顺序模型
...
我用 `refs/DRAMsim3` 源码编了一个最小 probe，测了 3 个场景：

- cold read
- row-hit read
- row-conflict read

`DRAMsim3` 在 `LPDDR4_8Gb_x16_2400.ini` 下的实测结果：

- cold read = `42`
- row-hit read = `27`
- row-conflict read = `57`
...
- row-hit write:
  `transaction_overhead + tCWL + burst_cycles = 2 + 14 + 8 = 24`
- cold write:
  `transaction_overhead + tRCD + tCWL + burst_cycles = 2 + 15 + 14 + 8 = 39`
- 这里的 `write_recovery = 30` 会影响后继请求何时能继续发出
- 它不会增加当前这一次 write 本身的 `total_cycles`
...
在当前默认的 LPDDR 参数下，这个 Python 模块现在可以认为：
