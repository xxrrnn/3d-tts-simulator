...
def run_CrossAttn_baseline(
    my_attn_layer: CrossAttn_Layer_Baseline,
    ...
    s_gen_matmul, load_gened_part_q_from_dram, load_gened_part_k_from_dram, store_psum_s_to_dram = my_attn_layer.psum_s_gen_per_core()
    
    s_allreduce_on_noc_per_core = my_attn_layer.s_allreduce_on_noc_per_core()
    
    sv_gen_matmul, load_s_from_dram, load_gened_part_v_from_dram, store_part_sv_to_dram  = my_attn_layer.sv_gen_per_core()
    ...
    s_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_gened_part_q_from_dram + load_gened_part_k_from_dram + store_psum_s_to_dram,
        dram_config=my_dram_config,
    )
    ...
    sv_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_s_from_dram + load_gened_part_v_from_dram + store_part_sv_to_dram,
        dram_config=my_dram_config,
    )
    ...
    non_linear_energy_consumption = 64 * (nnlut_for_softmax_latency.softmax_energy_consumption()/4 + layer_norm_latency_computer.layernorm_energy_consumption()/64)
    ...
    logger.info(f"Attn基线模型非线性能耗: {non_linear_energy_consumption}")
