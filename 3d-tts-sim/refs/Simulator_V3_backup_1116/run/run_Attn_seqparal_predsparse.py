...
def run_Attn_seqparal_predsparse(
    ...
    pred_s_gen_matmul, load_gened_chunk_q_from_dram, load_cached_chunk_k_from_dram, transfer_gened_chunk_k_from_noc, store_chunk_pred_s_to_dram = my_attn_layer.chunk_pred_s_gen_per_core()

    s_gen_matmul, load_gened_chunk_q_from_dram, load_cached_chunk_k_from_dram, transfer_gened_chunk_k_from_noc, store_chunk_s_to_dram = my_attn_layer.chunk_s_gen_per_core()
    
    sv_gen_matmul, load_chunk_s_from_dram, load_cached_chunk_v_from_dram, transfer_gened_chunk_v_from_noc, store_chunk_sv_to_dram  = my_attn_layer.sv_gen_per_core()
    ...
    pred_s_gen_latency_computer = CIM_Latency_Computer(
        matmul_workload=pred_s_gen_matmul,
        cim_config=my_cim_config,
        cim_num=my_cim_num,
    )

    topk_latency_computer = TopK_Latency_Computer(
        input_array_nums=my_attn_layer.generated_chunk_pred_s_per_core.matrix_height,
        input_array_length=my_attn_layer.concat_chunk_k_per_core.matrix_height,
        topk_config=my_topk_config,
        output_array_length=my_attn_layer.generated_chunk_s_per_core.matrix_width,
    )
    ...
    pred_s_gen_dram_latency_computer = DRAM_Latency_Computer(
        transfer_workload=load_gened_chunk_q_from_dram + load_cached_chunk_k_from_dram/4 + store_chunk_pred_s_to_dram,
        dram_config=my_dram_config,
    )

    pred_s_gen_noc_latency_computer = NoC_Latency_Computer(
        transfer_workload=transfer_gened_chunk_k_from_noc/4,
    ...
    s_gen_noc_latency_computer = NoC_Latency_Computer(
        transfer_workload=transfer_gened_chunk_k_from_noc,
        noc_config=my_noc_config,
    )
    ...
    sv_gen_noc_latency_computer = NoC_Latency_Computer(
        transfer_workload=transfer_gened_chunk_v_from_noc,
        noc_config=my_noc_config,
    )
    ...
    topk_latency = topk_latency_computer.topk_latency()
    non_linear_latency = nnlut_for_softmax_latency.softmax_latency() + layer_norm_latency_computer.layernorm_latency()
    computing_latency = q_gen_latency_computer.compute_latency() + k_gen_latency_computer.compute_latency() + v_gen_latency_computer.compute_latency()+ pred_s_gen_latency_computer.compute_latency()/4 + s_gen_latency_computer.compute_latency() + sv_gen_latency_computer.compute_latency() + o_gen_latency_computer.compute_latency()
    dram_latency = q_gen_dram_latency_computer.transfer_latency() + k_gen_dram_latency_computer.transfer_latency() + v_gen_dram_latency_computer.transfer_latency() + pred_s_gen_dram_latency_computer.transfer_latency() + s_gen_dram_latency_computer.transfer_latency() + sv_gen_dram_latency_computer.transfer_latency() + o_gen_dram_latency_computer.transfer_latency()
    noc_latency = q_all2all_noc_latency_computer.transfer_latency() + k_all2all_noc_latency_computer.transfer_latency() + v_all2all_noc_latency_computer.transfer_latency() + pred_s_gen_noc_latency_computer.transfer_latency() + s_gen_noc_latency_computer.transfer_latency() + sv_gen_noc_latency_computer.transfer_latency() + sv_all2all_noc_latency_computer.transfer_latency() + o_allreduce_noc_latency_computer.transfer_latency()

    total_latency = computing_latency + dram_latency + noc_latency + non_linear_latency + topk_latency

    logger.info(f"Attn序列并行+预测稀疏模型TopK延迟: {topk_latency}")
    logger.info(f"Attn序列并行+预测稀疏模型非线性延迟: {non_linear_latency}")
    logger.info(f"Attn序列并行+预测稀疏模型计算延迟: {computing_latency}")
    logger.info(f"Attn序列并行+预测稀疏模型DRAM延迟: {dram_latency}")
    logger.info(f"Attn序列并行+预测稀疏模型NoC延迟: {noc_latency}")
    
    logger.info(f"Attn序列并行+预测稀疏模型总延迟: {total_latency}")

    
    topk_energy_consumption = 64 * (my_topk_config.power/1000000) * topk_latency #uW ms
    non_linear_energy_consumption = 64 * (nnlut_for_softmax_latency.softmax_energy_consumption() + layer_norm_latency_computer.layernorm_energy_consumption())
    computing_energy_consumption = 64* (q_gen_latency_computer.compute_energy_consumption() + k_gen_latency_computer.compute_energy_consumption() + v_gen_latency_computer.compute_energy_consumption() + pred_s_gen_latency_computer.compute_energy_consumption()/4 + s_gen_latency_computer.compute_energy_consumption() + sv_gen_latency_computer.compute_energy_consumption() + o_gen_latency_computer.compute_energy_consumption())
    
    dram_energy_consumption = 64*(q_gen_dram_latency_computer.transfer_energy_consumption() + k_gen_dram_latency_computer.transfer_energy_consumption() + v_gen_dram_latency_computer.transfer_energy_consumption() + pred_s_gen_dram_latency_computer.transfer_energy_consumption() + s_gen_dram_latency_computer.transfer_energy_consumption() + sv_gen_dram_latency_computer.transfer_energy_consumption() + o_gen_dram_latency_computer.transfer_energy_consumption())
    noc_energy_consumption = 64*(q_all2all_noc_latency_computer.transfer_energy_consumption() + k_all2all_noc_latency_computer.transfer_energy_consumption() + v_all2all_noc_latency_computer.transfer_energy_consumption() + pred_s_gen_noc_latency_computer.transfer_energy_consumption() + s_gen_noc_latency_computer.transfer_energy_consumption() + sv_gen_noc_latency_computer.transfer_energy_consumption() + sv_all2all_noc_latency_computer.transfer_energy_consumption() + o_allreduce_noc_latency_computer.transfer_energy_consumption())
    
    sram_energy_consumption = 64 * total_latency * my_srambuffer_config.average_power

    total_energy_consumption = topk_energy_consumption + non_linear_energy_consumption + computing_energy_consumption + dram_energy_consumption + noc_energy_consumption + sram_energy_consumption

    logger.info(f"Attn序列并行+预测稀疏模型TopK能耗: {topk_energy_consumption}")
    logger.info(f"Attn序列并行+预测稀疏模型非线性能耗: {non_linear_energy_consumption}")
    logger.info(f"Attn序列并行+预测稀疏模型计算能耗: {computing_energy_consumption}")
    logger.info(f"Attn序列并行+预测稀疏模型DRAM能耗: {dram_energy_consumption}")
    logger.info(f"Attn序列并行+预测稀疏模型NoC能耗: {noc_energy_consumption}")
    logger.info(f"Attn序列并行+预测稀疏模型SRAM能耗: {sram_energy_consumption}")
    logger.info(f"Attn序列并行+预测稀疏模型总能耗: {total_energy_consumption}")
