...
setup_logger(root_dir, "run_Infinity2B_SP_PS")
...
def main_for_Infinity2B_SeqParal_PredSparse():
    logger.info("Infinity2B模型-序列并行+预测稀疏-模型运行开始")
    ...
    my_selfattn_layer_spps = Attn_Layer_SeqParal_PredSparse(
        stage_id=0,
        head_num=16,
        head_dim=128,   
        hidden_dim=2048,
    ...
    result_file_dir = os.path.join(result_dir, "infinity2B_SP_PS_1115.txt")
    
    with open(result_file_dir, "w") as f:   
        f.write(f"开始Infinity2B模型-序列并行+预测稀疏-模型运行\n")
    ...
    for i in range(Infinity2B_Model_config["layer_num"]):
        ...
        if i <= 4:
            ...
            latency, energy = run_TF_block_baseline(
            ...
            my_selfattn_layer_spps.set_N(my_input_token_num)
            my_selfattn_layer_spps.set_M(my_cached_token_num)
            my_selfattn_layer_spps.set_stage_id(i)
            my_selfattn_layer_spps.init_matrices()

            # 运行TF块序列并行模型
            latency, energy = run_TF_block_SeqParal_PredSparse(  
                my_crossattn_layer=my_crossattn_layer_sp,
                my_selfattn_layer=my_selfattn_layer_spps,
                my_ffn_layer=my_ffn_layer,
                my_cim_config=my_cim_config,
                my_cim_num=my_cim_num,
                my_dram_config=my_dram_config,
        ...
        stage_energy_consumption.append(energy)
        
        with open(result_file_dir, "a") as f:
            f.write(f"第{i}层延迟: {stage_latency[-1]}ms; 能耗: {stage_energy_consumption[-1]}mJ\n")
        
    logger.info(f"Infinity2B模型-序列并行+预测稀疏模型总延迟: {sum(stage_latency)}ms")
    logger.info(f"Infinity2B模型-序列并行+预测稀疏模型总能耗: {sum(stage_energy_consumption)}mJ")

    with open(result_file_dir, "a") as f:
        f.write(f"总延迟: {sum(stage_latency)}ms\n")
        f.write(f"总能耗: {sum(stage_energy_consumption)}mJ\n")
    


if __name__ == "__main__":
    main_for_Infinity2B_SeqParal_PredSparse()    
